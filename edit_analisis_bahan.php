...
$no_analisis1 = $_GET['ids'];
$no_analisis_bahan1 = $_GET['id'];
$no_proyek = $_GET['idp'];

if (!isset($_POST["submit"])){
	$query = " SELECT * FROM analisis_bahan WHERE no_analisis_bahan='".$no_analisis_bahan1."'";
	// Execute the query
	...
	if ( $valid_kodebarang && $valid_perkiraan){
		...
		//Asign a query
		$query = " UPDATE analisis_bahan SET  kode_barang='".$kode_barang."', perkiraan='".$perkiraan."' where no_analisis_bahan='".$no_analisis_bahan1."'";
		// Execute the query
		$result = $db->query( $query );
		if (!$result){
		   die ("Could not query the database: <br />". $db->error);
			...
			echo "<script>window.open('detail_analisis.php?id=".$no_analisis1."&idp=".$no_proyek."','_self')</script>";
			$db->close();
			exit;
		}
                                      ...
                                      <div class="col-sm-10">
                                              ...
                                              <?php
                                                  ...
                                                  while ($row = $resulttk->fetch_object()){
                                                      ...
                                                      if(isset($kode_barang) && $kode_barang==$tid)
                                                      echo "selected='true'";
                                                      echo ">".$tname."</option>";
                                                  }
                                              ?></select>
